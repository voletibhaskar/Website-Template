<?php
	...
	if (!empty($_SESSION['log_in']) && !empty($_SESSION['user']))
	{
		...
		$genre=$_POST['Genre'];
		$db=mysql_select_db("Music",$dbc);
		$update1="UPDATE Members set First_Name='$name' WHERE Email_ID='$username'";
		mysql_query($update1);
		$update2="UPDATE Members set Surname='$surname' WHERE Email_ID='$username'";
		mysql_query($update2);
		$update3="UPDATE Favorite set Artist='$artist' WHERE Email_ID='$username'";
		mysql_query($update3);
		$update4="UPDATE Favorite set Genre='$genre' WHERE Email_ID='$username'";
		mysql_query($update4);
