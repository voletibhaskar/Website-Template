<?php
	...
	if (!empty($_SESSION['log_in']) && !empty($_SESSION['user']))
	{
		...
		$db=mysql_select_db("Music",$dbc);
		$sql="DELETE FROM Members WHERE Email_ID='$username'";
		$result=mysql_query($sql);
		$sql1="DELETE FROM Favorite WHERE Email_ID='$username'";
		$result1=mysql_query($sql1);
		session_destroy();
		echo "<script>window.location='http://localhost/project/success_delete.php'</script>";
	}
	else
	{
?>
